...
| OQ-27 | **A rider already inside a cab that breaks down has nowhere to go.** A breakdown now ends the trip (aborted mid-way, cancelled before start) and riders not yet collected return to `queued` via the existing `assigned -> queued` edge. A rider who was already picked up has no legal transition: `picked_up` leads only to `dropped`, and marking them dropped would record a journey that did not happen. `trip-lifecycle.md` section 2 says such riders "get new handling by supervisor" without saying what that is. Options: a new `picked_up -> queued` edge for a re-ride; a distinct terminal status such as `stranded`; or a supervisor-only manual resolution. Needed for S05 to assert on rider outcomes rather than only on the trip | Phase 1 (M08) | Product | open |
