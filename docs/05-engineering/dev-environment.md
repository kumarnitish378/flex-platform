...
## 1. Prerequisites
- Linux or macOS (Windows via WSL2), 16 GB RAM recommended (OSRM + Nominatim for Delhi NCR).
- Docker + Docker Compose v2
- Python 3.12 + `uv` (or pip/venv)
- Flutter stable SDK + Android Studio / Android SDK; a physical Android phone for GPS tests
- `make`, `git`, `osmium-tool` (for cropping OSM data)
- Java 21 (only to run Planetiler for tile generation)
...
## 3. Map data preparation (one-time, then monthly)
...
| `OSRM_URL` | `http://localhost:5000` | backend, simulator |
| `NOMINATIM_URL` | `http://localhost:8080` | backend |
| `TILES_URL` | `http://localhost:8081` | app |
...
| `make sim-quick` / `make sim-full` | Simulator suites (starts backend with `APP_ENV=sim`) |
| `make maps` | Run `prepare_maps.sh` |
